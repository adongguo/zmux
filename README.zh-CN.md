...
<p align="center">
  <a href="../../releases/latest/download/zmux-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="下载 zmux macOS 版" width="180" />
  </a>
</p>

zmux 是 [cmux](https://github.com/manaflow-ai/cmux) 的独立发行分叉。应用、签名、更新源、回调协议、Keychain、状态目录、Socket、通知和发布资产均使用独立命名空间，因此可以与 cmux 并行安装和运行。

本发行版面向个人 Apple Developer 账号。公开稳定版与 Nightly 不包含内置浏览器或原生 Passkey/安全密钥桥接，也不会复用 cmux 的签名与更新凭据。zmux 的源码和问题反馈请使用[当前仓库](.)与[当前问题跟踪器](../../issues)；下文 Manaflow 链接仅作为上游来源与致谢保留。
...
- **可脚本化** — 通过 CLI 和 socket API 创建工作区、分割窗格、发送按键和读取终端状态
...
打开 `.dmg` 文件并将 zmux 拖动到“应用程序”文件夹。zmux 通过分叉自有的 Sparkle 更新源自动更新。
...
终端和工作区操作都可以通过 CLI 与 socket API 脚本化：创建工作区/标签页、分割窗格、发送按键并读取终端状态。

## The Zen of cmux

cmux 不规定开发者应该如何使用工具；公开 zmux 保留了其中的终端、工作区、通知和 CLI 能力。
...
[下载 zmux NIGHTLY](../../releases/download/nightly/zmux-nightly-macos.dmg)

zmux NIGHTLY 是一个拥有独立 Bundle ID 的单独应用，因此可以与稳定版并行运行。它从 `zmux` 分支的最新提交自动构建，并通过独立的 Sparkle 更新源自动更新。

请在当前分叉的 [GitHub Issues](../../issues) 中报告每夜构建问题。
...
可以。终端与工作区操作可通过 zmux CLI 和 Unix socket 使用：创建工作区、打开分割窗格、发送输入、读取屏幕内容和截图。

### 为什么内置浏览器不可用？

公开 zmux 版本面向个人 Apple Developer 分发，因此移除了受管 WebAuthn entitlement 和原生 Passkey 桥接，并关闭所有内置浏览器入口。网页链接和账号登录改由系统默认浏览器处理；认证回调仍使用产品自有的 `zmux://auth-callback`。

### cmux 有 skills 吗？

有。Skills 是可复用工作流，可用于 zmux 的 CLI 控制、工作区自动化和设置。上游 [cmux-skills](https://github.com/manaflow-ai/cmux-skills) 中可能包含公开 zmux 版本并未提供的浏览器能力说明。

### 我可以自定义键盘快捷键吗？

终端键绑定从您的 Ghostty 配置文件（`~/.config/ghostty/config`）中读取。zmux 专属的工作区、分割和通知快捷键可在设置中自定义。
...
tmux 是一个在任意终端内运行的终端复用器。zmux 是一个带垂直标签页、分割窗格、通知和 socket API 的原生 macOS 应用，也可以与 SSH 和 tmux 配合使用。
...
请在当前分叉提交 [issue](../../issues) 或 pull request。只有能在原始 cmux 项目复现的问题才应提交到上游跟踪器。
...
zmux 按 [GPL-3.0-or-later](LICENSE) 分发，并保留上游及第三方版权声明。向接收者分发二进制时，必须按许可证提供对应源码。Manaflow 可能提供的商业条款仅覆盖其有权许可的部分，不能替代此独立分叉的 GPL 义务。
