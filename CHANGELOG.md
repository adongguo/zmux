...
### Added
- 将产品完整分叉为 zmux，使用独立的应用身份、配置、Socket、CLI 和 Developer ID 公证 DMG 分发渠道，不会覆盖 cmux 或本地开发版 — 感谢 @zhuoyue.gxd！
- 新增 zellij 风格的嵌套子标签页，以及重命名、关闭、移动和分组级上下文菜单 — 感谢 @zhuoyue.gxd！
- 新增本地终端会话持久化，应用重启和版本更新后可重新连接原有终端 — 感谢 @zhuoyue.gxd！
- 新增右侧边栏的 Agent Activity 与 Git Changes 面板，并支持自定义面板按钮和快捷键 — 感谢 @zhuoyue.gxd！
- 新增 Quick Access、Running Agents 和统一任务状态，让运行中、等待输入及已完成的 Agent 更容易定位 — 感谢 @zhuoyue.gxd！
- 新增工作区外部链接与笔记，可通过侧边栏、命令面板、CLI 和 Socket 管理 — 感谢 @zhuoyue.gxd！
- 新增 `Cmd+E` 最近项目面板，以及从标签栏、快捷键和命令面板创建 Agent Session 的入口，并支持 Qoder — 感谢 @zhuoyue.gxd！
- 文件预览新增语法高亮，并可直接使用 Vim 或 Neovim 打开 — 感谢 @zhuoyue.gxd！

### Changed
- 默认界面语言调整为英文，并统一应用、CLI、欢迎页和诊断信息中的 zmux 品牌名称 — 感谢 @zhuoyue.gxd！
- 侧边栏字体、标题粗细、选择色、面板顺序与可见性支持配置，通知铃铛可从标题栏直接跳转 — 感谢 @zhuoyue.gxd！
- 安全的纯文本粘贴改走低延迟路径，提升大多数终端粘贴操作的响应速度 — 感谢 @zhuoyue.gxd！

### Fixed
- 修复恢复中的持久终端被误删、旧连接回调覆盖新运行时、已删除工作目录导致新标签页退出，以及 daemon Socket 丢失后的恢复问题 — 感谢 @zhuoyue.gxd！
- 修复重新连接终端时历史 OSC 通知和铃铛被重复触发的问题 — 感谢 @zhuoyue.gxd！
- 修复 Agent 输出超过 64 KiB 时读取阻塞，以及 Agent 状态在清屏、中断和等待输入后的错误显示 — 感谢 @zhuoyue.gxd！
- 修复侧边栏首行无法拖动、镜像行展开相邻分组、多选颜色不一致及任务状态圆点显示错误 — 感谢 @zhuoyue.gxd！
- 修复最近关闭标签页显示错误工作区、工作区分组锚点丢失及子分组排序异常 — 感谢 @zhuoyue.gxd！
- 修复版本更新后远程与本地持久会话失联，以及并行能力集错误复用 daemon 命名空间的问题 — 感谢 @zhuoyue.gxd！
